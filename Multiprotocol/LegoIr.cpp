...
// Combo PWM mode
int LegoIr::pwm_convert(uint8_t convert_speed) {
		 if (convert_speed <  6)	return  PWM_REV7;
	else if (convert_speed < 12)	return  PWM_REV6;
	else if (convert_speed < 18)	return  PWM_REV5;
	else if (convert_speed < 24)	return  PWM_REV4;
	else if (convert_speed < 30)	return  PWM_REV3;
	else if (convert_speed < 36)	return  PWM_REV2;
	else if (convert_speed < 42)	return  PWM_REV1;
	else if (convert_speed < 58)	return  PWM_BRK;
	else if (convert_speed < 64)	return  PWM_FWD1;
	else if (convert_speed < 70)	return  PWM_FWD2;
	else if (convert_speed < 76)	return  PWM_FWD3;
	else if (convert_speed < 82)	return  PWM_FWD4;
	else if (convert_speed < 88)	return  PWM_FWD5;
	else if (convert_speed < 94)	return  PWM_FWD6;
	else 							return  PWM_FWD7;
}
...
void LegoIr::pause(uint8_t count) {
	uint8_t pause = 0;
	if(count == 0) {		pause = 4 - (_channel + 1);	}
	else if(count < 3) {	pause = 5; } 	// 1, 2
	else {					pause = 5 + (_channel + 1) * 2;}	// 3, 4, 5
